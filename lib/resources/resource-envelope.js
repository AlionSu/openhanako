...
export function createSessionFileResourceEnvelope(file, { spaceId } = {}) {
  const source = file && typeof file === "object" ? file : {};
  if (typeof spaceId !== "string" || !spaceId.trim()) return null;
  ...
  const envelope = {
    ...
    resourceId,
    name: `spaces/${spaceId}/resources/${resourceId}`,
    spaceId,
    type: "file",
    source: "session_file",
    sourceId: fileId,
